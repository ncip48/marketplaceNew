...
export default class BottomSheet extends React.Component {
  constructor(props) {
    super(props);
    this.popUpHeight = this.props.height
      ? this.props.height
  ...
  createPanResponder() {
    ...
    this.panResponder = PanResponder.create({
      ...
      onPanResponderRelease: (e, gesture) => {
        const shouldOpen =
          gesture.vy < 0.8 && gesture.dy < this.popUpHeight * 0.45; //popup is opening or closing by swipe.(decision based on swiping speed and siped distance)
        Animated.spring(this.state.popupPosition, {
          //animate popup position w.r.t swipe value
  ...
  open = (callback) => {
    ...
    Animated.timing(this.state.popupPosition, {
      //animate popup position to slide up
      toValue: 0,
      duration: 250,
      useNativeDriver: true,
    }).start(() => {
  ...
  close = (callback) => {
    ...
    Animated.timing(this.state.popupPosition, {
      //animate popup position to slide down
  ...
  render() {
    const backdrop = {
      //animates backDrop position & opacity
      transform: [
        {
    ...
    return (
      <Animated.View
        style={[styles.backDropView, this.props.backDropStyle, backdrop]}>
        <TouchableOpacity
          activeOpacity={1}
          style={[styles.backDropTouchable]}
          onPress={
            this.props.closeOnPressMask !== false ? () => this.close() : null
          }
        />

        <Animated.View
          {...(this.props.closeOnDragDown !== false
